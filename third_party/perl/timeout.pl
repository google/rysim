...
while ($status eq 'wait'){
	...
	print STDOUT Dumper($uinfo) if $debug;
	print STDOUT Dumper($timeinfo) if $debug;
	$child_errno = $!;
	$child_retv = $?;
	my $arrived = -1;
	...
	if ($arrived == $blackbox_pid){
		...
		if (my $reason = limits_exceeded()){
			...
			print_uinfo($reason,$uinfo);
                        if (($reason eq 'TIMEOUT') || ($reason eq 'MEM')) {
                            exit -1;
                        }
...
sub foreach_applicable_process
{
	...
	if ($watchfor eq 'tree') {
		...
		# 26944 12496 kmail
		my $chars = "\t \\_|";
		my $PS_FH; open $PS_FH, "-|", qw(ps -e f -o pgrp= -o pid= -o sz= -o ucmd=) or die "Bad open ps: $!";
		...
		while (<$PS_FH>){
			...
			my ($grp,$pid,$sz,$depth_str,$cmd) = ($1,$2,$3,$4,$5);
			if ($state == 0){
		...
		my $PS_FH; open $PS_FH, "-|", qw(ps -A -o pgrp= -o pid= -o sz= -o ucmd=) or die "Bad open ps: $!";
		while (<$PS_FH>){
			/^\s*([0-9]+)\s*([0-9]+)\s*([0-9]+)\s*(.*)/ or next;
			my ($grp,$pid,$sz,$cmd) = ($1,$2,$3);
			$grp == $pgrp or next;
...
sub update_time
{
	...
	foreach_applicable_process($pgrp,$watchfor,sub { my ($pid,$grp,$cmd) = @_;
		...
		if (defined $pid_time){
			printf STDOUT "timeout: pid $pid own $pid_time kids $pid_cum_time\n" if $debug;
...
sub update_memory
{
	...
	my ($pgrp) = @_;
	my $result = 0;
	foreach_applicable_process($pgrp,$watchfor,sub { my ($pid,$grp,$cmd,$sz) = @_;
		$result += $sz;
	});
	return $result;
}
...
sub kill_process_group_safely
{
	...
	$SIG{'ALRM'} = 'DEFAULT';
	print STDOUT "timeout: Sending TERM\n" if $debug;
	signal_to_process_group_safely($pgrp,SIGTERM) unless $just_kill;
	sleep(1);
	print STDOUT "timeout: Sending KILL\n" if $debug;
...
{
	my $reason = shift;
        # Print generic information to STDOUT
	my $ticks = $timeinfo->{ticks_stale} || 0;
	printf STDOUT "CPU: %d\nMAXMEM: %d\n",$timeinfo->{total} * 1000, $maxmem if ($reason ne 'FINISHED') || $info_on_success;
#	printf STDOUT "${id_str}%s CPU %.2f MEM %d MAXMEM %d STALE %d\n", $reason, $timeinfo->{total}, $meminfo, $maxmem, ceil($ticks/$frequency) if ($reason ne 'FINISHED') || $info_on_success;
...
sub get_patterns
{
	...
	if ($patterns_in_string){
		...
		foreach (@splitted_patterns) {
			...
			printf STDOUT "timeout: pattern $pattern for bucket $name initialized\n" if $debug;
...
sub limits_exceeded
{
	if (defined $timelimit && $timeinfo->{total} * 1000 > $timelimit){
                print STDOUT "Time limit reached, stopping execution\n";
		return 'TIMEOUT';
	...
	# Sometimes the controlling process may inherently hang up.  Then we don't interrupt it.
		return 'HANGUP';
                ...
                print STDOUT "Memory limit reached, stopping execution\n";
		return 'MEM';
	}
	return undef;
}
