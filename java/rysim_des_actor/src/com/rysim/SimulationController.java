/*
 * Copyright 2014 The RySim Authors. All rights reserved.
 *
 * Licensed under the Apache License, Version 2.0 (the "License");
 * you may not use this file except in compliance with the License.
 * You may obtain a copy of the License at
 *
 * http://www.apache.org/licenses/LICENSE-2.0
 *
 * Unless required by applicable law or agreed to in writing, software
 * distributed under the License is distributed on an "AS IS" BASIS,
 * WITHOUT WARRANTIES OR CONDITIONS OF ANY KIND, either express or implied.
 * See the License for the specific language governing permissions and
 * limitations under the License.
 */
package com.rysim;

import akka.actor.ActorRef;
import akka.actor.Props;
import akka.actor.UntypedActor;
import org.json.simple.JSONObject;

public class SimulationController extends UntypedActor {
    private static final String TAG = "SimulationController";

    private String mExperimentName;
    private String mExperimentType;
    private long mEventLimit;
    private final Model mModel = new Model();
    private long mTimestamp = 0;
    private long mEventCount = 0;
    private ActorRef mEventQueue;
    private long mDirtyCount;
    private ActorRef mMasterActor;
    private long mInitializedCount = 0;
    private boolean mRunCalled = false;

    public static class InitMessage {
        public String filename;
        public long seed;
        public static InitMessage create(String filename, long seed) {
            InitMessage message = new InitMessage();
            message.filename = filename;
            message.seed = seed;
            return message;
        }
    }

    public static class RunMessage {
        public static RunMessage create() {
            return new RunMessage();
        }
    }

    public static class PrintMessage {
        public static PrintMessage create() {
            return new PrintMessage();
        }
    }

    public static class AdvancedMessage {
        public static AdvancedMessage create() { return new AdvancedMessage(); }
    }

    public static class InitializedMessage {
        public static InitializedMessage create() { return new InitializedMessage(); }
    }
    public SimulationController() { }

    @Override
    public void onReceive(Object message) throws Exception {
        if (message instanceof InitMessage) {
            handleInitMessage((InitMessage) message);
        } else if (message instanceof RunMessage) {
            handleRunMessage();
        } else if (message instanceof PrintMessage) {
            handlePrintMessage();
        } else if (message instanceof AdvancedMessage) {
            handleAdvancedMessage();
        } else if (message instanceof InitializedMessage) {
            handleInitializedMessage();
        } else {
            unhandled(message);
        }
    }

    public long getAgentCount() {
        return mModel.getModelSize();
    }

    public long getEventLimit() {
        return mEventLimit;
    }

    public long getEventCount() {
        return mEventCount;
    }

    public long getTimestamp() {
        return mTimestamp;
    }

    private void handleInitMessage(InitMessage message) {
        boolean result = init(message.filename, message.seed);
        getSender().tell(result, getSelf());
    }

    private void handleRunMessage() {
        mRunCalled = true;
        mMasterActor = getSender();
        if (mInitializedCount == mModel.getModelSize())
            run();
    }

    private void handlePrintMessage() {
        printResults();
        getSender().tell(true, getSelf());
    }

    private void handleAdvancedMessage() {
        mDirtyCount--;
        if (mDirtyCount == 0)
            run();
    }

    private void handleInitializedMessage() {
        mInitializedCount++;
        if (mInitializedCount == mModel.getModelSize() && mRunCalled)
            run();
    }

    private boolean init(String filename, long seed) {
        mEventQueue = getContext().actorOf(Props.create(EventQueue.class), "EventQueue");
        NumberGenerator.getInstance().init(seed);
        mModel.init(getContext(), getSelf(), mEventQueue);

        final Experiment experiment = new Experiment();
        if (!experiment.parse(filename)) {
            SimpleLogger.error(TAG, "Unable to parse supplied experiment file = " + filename);
            return false;
        }
        mExperimentName = experiment.getExperimentName();
        mExperimentType = experiment.getExperimentType();
        mEventLimit = experiment.getEventLimit();

        if (!mModel.parse(experiment.getModelFilename())) {
            SimpleLogger.error(TAG, "Unable to parse supplied experiment file = " + experiment.getModelFilename());
            return false;
        }

        return true;
    }

    private void run() {
        Boolean booleanResult = null;
        if (mEventCount < mEventLimit
                && !MessagingUtils.askSynchronous(mEventQueue, EventQueue.IsEmptyMessage.create(), booleanResult)) {
            Event event = null;
            event = MessagingUtils.askSynchronous(mEventQueue, EventQueue.PopMessage.create(), event);
            mModel.processEvent(event);
            mEventCount++;
            mTimestamp = event.getTimestamp();
            while (MessagingUtils.askSynchronous(mEventQueue, EventQueue.IsNextEventAtMessage.create(mTimestamp), booleanResult)) {
                event = MessagingUtils.askSynchronous(mEventQueue, EventQueue.PopMessage.create(), event);
                mModel.processEvent(event);
                mEventCount++;
            }
            mDirtyCount = mModel.advanceState(mTimestamp);
        } else {
            mMasterActor.tell(true, getSelf());
        }
    }

    @SuppressWarnings("unchecked")
    private void printResults() {
        JSONObject jsonObject = new JSONObject();
        jsonObject.put("experiment", mExperimentName);
        jsonObject.put("type", mExperimentType);
        jsonObject.put("model", mModel.getModelName());
        jsonObject.put("kernel", "JavaDESActor");
        jsonObject.put("event_count", mEventCount);
        jsonObject.put("final_time", mTimestamp);
        jsonObject.put("agents", mModel.getModelSize());
        jsonObject.put("connections", mModel.getTotalConnections());

        System.out.println("JSONBEGIN");
        System.out.println(jsonObject);
        System.out.println("JSONEND");
    }

}
